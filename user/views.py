...
def to_github(request):
    ...
    return redirect(
        f'https://github.com/login/oauth/authorize?client_id={GITHUB_ID}&redirect_uri={REDIRECT_URI}&scope=read:user user:email')
...
def from_github(request):
    ...
    try:
        ...
        user = models.CustomUser.objects.create(username=username, nickname=nickname, profile_img=profile_img,
                                                email=email, login_method=models.CustomUser.LOGIN_GITHUB)
...
def from_kakao(request):
    ...
    try:
        ...
        user = models.CustomUser.objects.create(username=username, nickname=nickname, profile_img=profile_img,
                                                email=email, login_method=models.CustomUser.LOGIN_KAKAO, gender=gender)
        user.set_unusable_password()
        user.save()
...
def email_ajax(request):
    ...
    if request.method == 'POST':
        ...
        send_mail('레트로 플릭스 회원가입 인증 메일입니다.',
                  f'안녕하세요 아래의 인증번호를 입력해주세요\n\n인증번호 : {certify_num}', 'taehyeki123@gmail.com', [email],
                  fail_silently=False)
        context = {
...
def is_id(request):
    ...
    try:
        ...
        send_mail('레트로 플릭스 임시 비밀번호 메일입니다.',
                  ...
                  'taehyeki123@naver.com', [email], fail_silently=False)
        context = {
